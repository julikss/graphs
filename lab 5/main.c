...
int *p[11]; int spos = 0; int rpos = 0; int typeALG;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam)
    ...
    //int *p[11]; int spos = 0; int rpos = 0;
    //Функции очереди
    //Добавление очереди
    void qstore(int *q) {
        if(spos==11) {
    ...
    void BFS (HDC hdc, double **A, int a, int *nx, int *ny){
        ...
        HPEN RPen = CreatePen(PS_SOLID, 1, RGB(220, 20, 60));
    ...
    switch(messg){
        ...
        double coefficient = 1 - 1*0.001 - 4*0.005 - 0.15;
